var HivePlot = function(chartElementId, infoElementId, nodes)
{
	...
	var svg = d3.select("#"+chartElementId).append("svg")
	  ...
	  svg.selectAll(".axis")
		...
		.enter().append("line")
		  .attr("class", "axis")
		  .attr("transform", function(d) { return "rotate(" + HivePlot_degrees(angle(d.key)) + ")"; })
		  .attr("x1", radius(-2))
	  ...
	  svg.append("g")
		...
		.enter().append("path")
		  .attr("class", "link")
		  .attr("d", HivePlot_link()
		  .angle(function(d) { return angle(d.type); })
		  .radius(function(d) { return radius(d.node.index); }))
		  .on("mouseover", HivePlot_linkMouseover)
		  .on("mouseout", HivePlot_mouseout);
	  ...
	  svg.append("g")
		...
		.enter().append("circle")
		  .attr("transform", function(d) { return "rotate(" + HivePlot_degrees(angle(d.type)) + ")"; })
		  .attr("cx", function(d) { return radius(d.node.index); })
		  .attr("r", 4)
		  .on("mouseover", HivePlot_nodeMouseover)
		  .on("mouseout", HivePlot_mouseout);

	  // Highlight the link and connected nodes on mouseover.
	  function HivePlot_linkMouseover(d) {
	  ...
	  function HivePlot_nodeMouseover(d) {
